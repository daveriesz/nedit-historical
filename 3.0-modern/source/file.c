...
static int doSave(WindowInfo *window)
{
    ...
    */
    ...
    sprintf(bigbuffer, "%s~%s", path, window->filename);
    strncpy(tempname, bigbuffer, MAXPATHLEN-1);
...
#ifdef VMS
    sprintf(bigbuffer, "%s\_%s", window->path, window->filename);
#else
    sprintf(bigbuffer, "%s~%s", window->path, window->filename);
#endif /*VMS*/
    strncpy(name, bigbuffer, MAXPATHLEN-1);
