...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)search.c	1.11     3/8/94";
#include <stdio.h>
#include <ctype.h>
#ifdef VMS
...
static int storeReplaceDlogInfo(WindowInfo *window)
{
	...
	if (compiledRE == NULL) {
   	    DialogF(DF_WARN, window->replaceText, 1,
...
static int storeFindDlogInfo(WindowInfo *window)
{
	...
	if (compiledRE == NULL) {
   	    DialogF(DF_WARN, window->findText, 1,
   	    	   "Please respecify the search string:\n%s", "OK", compileMsg);
 	    return FALSE;
 	}
...
int SearchAndSelect(WindowInfo *window, int direction, Time time)
{
    ...
    MakeSelectionVisible(window->lastFocus);
...
int ReplaceInSelection(WindowInfo *window)
{
	...
	if (found) {
		...
		XtFree(foundString);
    	    ...
    	    beginPos = (beginPos == endPos) ? endPos+1 : endPos;
	    anyFound = TRUE;
	}
    }
    XtFree(fileString);
