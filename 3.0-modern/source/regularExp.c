 ...
 */
static char SCCSID[] = "@(#)regularExp.c	1.2     2/23/94";
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <ctype.h>
...
static char *startofstring; /* Beginning of input, for ^ check. */
 ...
 * to find whether there was a newline before).
 */
int
ExecRE(prog, string, end, reverse, isbol)
...
int isbol;
{
    ...
    startofstring = isbol ? string : NULL;
...
char *prog;
{
	...
	switch (OP(scan)) {
	case BOL:
	    if (reginput != startofstring && *(reginput-1) != '\n')
		return(0);
	    break;
	case EOL:
	    if (*reginput != '\n' && *reginput != '\0')
 ...
 */
void
SubstituteRE(prog, source, dest, max)
regexp *prog;
char *source;
char *dest;
int max;
