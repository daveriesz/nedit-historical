...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)DialogF.c	1.7	2/23/94";
#include <stdio.h>
#include <Xm/Xm.h>
		...
		char *msgstr, ...)		/* variable # arguments */
{
  ...
  XtAddCallback(
	...
	XtManageChild(dialog);		/* Pop it up, wait for response */
...
{
  XtVaSetValues(dialog,
    XmNdefaultButton, XmSelectionBoxGetChild(dialog, XmDIALOG_OK_BUTTON),
    NULL);
}
