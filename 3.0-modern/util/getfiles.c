...
*                                                                             *
******************************************************************************/
static char SCCSID[] = "@(#)getfiles.c	1.20	3/2/94";
#include <stdio.h>
#include <string.h>
#ifdef VMS
#include <unixio.h>
...
You can also directly type in a directory and file specifications in the \
field labeled \"Filter\".\n\
\n\
If you use the filter field, remember to include \
either a file specification, \"*\" is acceptable, or a trailing \"/\".  If \
...
int HandleCustomExistFileSB(Widget existFileSB, char *filename)
{
    ...
    char      name[MAXPATHLEN];       /* name part of file specification   */
    char      path[MAXPATHLEN];       /* path part of file specification   */
    int i;
    ...
    {
      XtVaGetValues(existFileSB,
        XmNdirSpec, &cFileString,
        NULL);
      XmStringGetLtoR(cFileString, XmSTRING_DEFAULT_CHARSET, &fileString);
      strcpy(filename, fileString);
      XtFree(fileString);             /* jmk - 6/25/93                */
      ParseFilename(filename, name, path);
      chdir(path);
    }
...
int GetNewFilename (Widget parent, char *promptString, char *filename)
{
    ...
    if (SelectResult == GFN_OK) {
	n = 0;
	XtSetArg(args[n], XmNdirSpec, &cFileString); n++;
	XtGetValues(newFileSB, args, n);
	XmStringGetLtoR(cFileString, XmSTRING_DEFAULT_CHARSET, &fileString);
	strcpy(filename, fileString);
        XtFree(fileString);             /* jmk - 6/25/93                */     
    }
