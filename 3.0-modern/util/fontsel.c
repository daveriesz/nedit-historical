...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)fontsel.c	1.5	12/14/93";
...
char 	*FontSel(Widget parent, int showPropFonts, char *currFont)
{
	...
	{
		n = 0; 
		tempStr = XmStringCreate("Show proportional width fonts also", 
						      XmSTRING_DEFAULT_CHARSET);
		XtSetArg(args[n], XmNlabelString, tempStr); n++;
		XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION); n++; 
	...
	/*	kludge to link Motif Close option with cancel action */
...
static void	getFontPart(char *font, char *buff1)
{
  ...
  char huge_buffer[TEMP_BUF_SIZE * 10];


	getStringComponent(font, 2, buff1);
	getStringComponent(font, 1, buff2);

	sprintf(huge_buffer, "%s (%s", buff1, buff2);
  memset(buff3, 0, TEMP_BUF_SIZE);
  strncpy(buff3, huge_buffer, TEMP_BUF_SIZE-1);
	     ...
	     (strncmp(buff1, "ISO8859", 7) == 0)) && (strcmp(buff4, "1") == 0))
  {
	...
	{
		sprintf(huge_buffer, "%s, %s,", buff3, buff1);
    memset(buff2, 0, TEMP_BUF_SIZE);
    strncpy(buff2, huge_buffer, TEMP_BUF_SIZE-1);
		sprintf(huge_buffer, "%s %s)", buff2, buff4);
    memset(buff1, 0, TEMP_BUF_SIZE);
    strncpy(buff1, huge_buffer, TEMP_BUF_SIZE-1);
	}
}
