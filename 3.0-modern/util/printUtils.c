...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)printUtils.c	1.15	3/8/94";
...
static void updatePrintCmd(Widget w, caddr_t client_data, caddr_t call_data)
{
    ...
    char big_buffer[MAX_CMD_STR * 10];
    ...
    sprintf(big_buffer, "%s%s%s%s%s", PrintCommand, copiesArg,
    	    queueArg, hostArg, jobArg);
    memset(command, 0, MAX_CMD_STR);
    strncpy(command, big_buffer, MAX_CMD_STR-1);
