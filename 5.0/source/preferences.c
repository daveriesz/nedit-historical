...
int CheckPrefsChangesSaved(Widget dialogParent)
{
    ...
    resp = DialogF(DF_WARN, dialogParent, 3,
...
#define H_MARGIN 5
    ...
    if (LMDialog.shell != NULL) {
    	XMapRaised(TheDisplay, XtWindow(LMDialog.shell));
    	return;
    }
    	...
    	void *cbArg)
{
    ...
    /* If there was a name change, modify the name to the weird format of:
       ":old name:new name".  This signals that a name change is necessary
       in lm dependent data such as highlight patterns. */
    if (lm != NULL && oldLM != NULL && strcmp(oldLM->name, lm->name)) {
    	int oldLen = strchr(oldLM->name, ':') == NULL ? strlen(oldLM->name) :
...
#define MARGIN_SPACING 10
    ...
    if (window->fontDialog != NULL) {
    	XMapRaised(TheDisplay,
    	    	XtWindow(((fontDialog *)window->fontDialog)->shell));
    	return;
    }
...
static int checkFontStatus(fontDialog *fd, Widget fontTextFieldW)
{
    ...
    /* Get width and height of the font to check */
...
{    
	...
	if (*inPtr == '\n' || *inPtr == '\0')
   	...
   	/* pattern set was read correctly, copy it to the list and
