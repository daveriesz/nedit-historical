...
Ivan Herman, Patrice Venant, Christian Denat, and Philippe Couton.\n\
\n\
...
Repeat\n\
\n\
Individual actions or Learn/Replay sequences can be repeated any number of \
times by using the Repeat command in the Macro menu.  By default, Repeat \
repeats the last (keyboard-based) command entered.  For example, to move \
down 30 lines through a file, you could type: <Down Arrow> Ctrl+, 29 <Return>",
...
\"right\".\n\
\n\
set_cursor_pos() -- Set the cursor position for the current window.\n\
\n\
...
void Help(Widget parent, enum HelpTopic topic)
{
    if (HelpWindows[topic] != NULL)
    	XMapRaised(TheDisplay, XtWindow(HelpWindows[topic]));
    else
