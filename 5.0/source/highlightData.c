...
#ifdef VMS
#define N_DEFAULT_PATTERN_SETS 18
#else
#define N_DEFAULT_PATTERN_SETS 19
#endif /*VMS*/
...
/* The VAX C compiler cannot compile this definition */
	...
	Naked Variable Cmds:\"(set|setenv|shift)[ \\t]+[0-9a-zA-Z_]*(\\[.+\\])?\":::Identifier1::\n\
	...
	Pattern escape:\"\\\\/\":::Preprocessor:Pattern:\n\
	...
	Command:\"[_^]|[\\\\@]([A-Za-z]+\\*?|[-@_='`^\"\"~.])\":\"nevermatch\":\"[^{[(]\":Text Key::\n\
...
{    
	...
	XtFree(fontStr);
   	...
   	/* pattern set was read correctly, add it to the list and
   	   move on to the next pattern set */
...
int LoadHighlightString(char *inString)
{
    ...
    for (;;) {
   	...
   	/* Read each pattern set, abort on error or max # exceeded */
   	PatternSets[NPatternSets] = readPatternSet(&inPtr);
   	if (PatternSets[NPatternSets] == NULL)
   	    return False;
   	NPatternSets++;
...
#define HS_H_MARGIN 10
	...
	if (initialStyle != NULL)
    	...
    	XMapRaised(TheDisplay, XtWindow(HSDialog.shell));
    	return;
    }
...
#define LIST_RIGHT 41
    ...
    /* if the dialog is already displayed, just pop it to the top and return */
    if (HighlightDialog.shell != NULL) {
    	XMapRaised(TheDisplay, XtWindow(HighlightDialog.shell));
    	return;
    }
