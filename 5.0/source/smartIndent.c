...
# followed (ignoring blank lines and comments) by what we assume is a\n\
# properly indented line, or a brace on a line by itself.  Returns the\n\
# position of the first non-white, non comment character on the line.\n\
# returns -1 if an anchor position can't be found before $cMaxSearchBackLines.\n\
#\n\
...
\n\
	    ...
	    if (lineEnd == -1 || get_character(lineEnd) != \"\\n\")\n\
   		continue\n\
\n\
...
\n\
    ...
    if (lastChar == \"{\")\n\
	return anchorIndent + $cIndentDist\n\
    else if (lastChar == \"}\")\n\
	return anchorIndent\n\
    else if (lastChar == \";\") {\n\
	if ($allowSemi)\n\
...
\n\
	...
	return $cIndentDist  	#   probably quoted or extra braces\n\
\n\
    ...
    if (newAnchor == maxPos)\n\
	return $cIndentDist\n\
    return cCalcContinueIndent(newAnchor, maxPos) + $cIndentDist\n\
}\n\
";
...
#define BORDER 4
    ...
    if (SmartIndentDialog.shell != NULL) {
    	XMapRaised(TheDisplay, XtWindow(SmartIndentDialog.shell));
    	return;
    }
...
static int checkSmartIndentDialogData(void)
{
	...
	widgetText = XmTextGetString(SmartIndentDialog.initMacro);
    ...
    widgetText = XmTextGetString(SmartIndentDialog.newlineMacro);
    ...
    if (!TextWidgetIsBlank(SmartIndentDialog.modMacro)) {
    	widgetText = XmTextGetString(SmartIndentDialog.modMacro);
    	prog = ParseMacro(widgetText, &errMsg, &stoppedAt);
	if (prog == NULL) {
    	    ParseError(SmartIndentDialog.shell, widgetText, stoppedAt,
    	    	    "modify macro", errMsg);
...
static smartIndentRec *getSmartIndentDialogData(void)
{
    ...
    is->initMacro = TextWidgetIsBlank(SmartIndentDialog.initMacro) ? NULL :
	    XmTextGetString(SmartIndentDialog.initMacro);
    is->newlineMacro = TextWidgetIsBlank(SmartIndentDialog.newlineMacro) ?
	    NULL : XmTextGetString(SmartIndentDialog.newlineMacro);
    is->modMacro = TextWidgetIsBlank(SmartIndentDialog.modMacro) ? NULL :
	    XmTextGetString(SmartIndentDialog.modMacro);
    return is;
}
...
#define VERT_BORDER 4
    ...
    if (CommonDialog.shell != NULL) {
    	XMapRaised(TheDisplay, XtWindow(CommonDialog.shell));
    	return;
    }
...
static int updateSmartIndentCommonData(void)
{
    ...
    CommonMacros = XmTextGetString(CommonDialog.text);
...
static int checkSmartIndentCommonDialogData(void)
{
	...
	widgetText = XmTextGetString(CommonDialog.text);
...
int LoadSmartIndentString(char *inString)
{
   ...
   for (;;) {
	...
	if (macroStart == inPtr) {
    	...
    	/* create a new data structure and add it to the list */
	SmartIndentSpecs[NSmartIndentSpecs] =
		(smartIndentRec *)XtMalloc(sizeof(smartIndentRec));
	*SmartIndentSpecs[NSmartIndentSpecs++] = is;
    }
}
