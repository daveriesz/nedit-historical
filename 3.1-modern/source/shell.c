...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)shell.c	1.9     9/1/94";
#include <stdio.h>
#include <sys/param.h>
#include <sys/wait.h>
#include <sys/time.h>
...
/* major divisions (in position units) in Shell Commands dialog */
#define LIST_LEFT 57
#define CMD_TOP 70
...
static Widget NameTextW, AccTextW, MneTextW, CmdTextW, ListW, SaveFirstBtn;
static Widget LoadAfterBtn, SelInpBtn, WinInpBtn, EitherInpBtn, NoInpBtn;
static Widget SameOutBtn, DlogOutBtn, WinOutBtn;
...
void FilterSelection(WindowInfo *window)
{
    ...
    issueCommand(window->shell, cmdText, text, ACCUMULATE | ERROR_DIALOGS,
    	    window->lastFocus, left, right, &success);
...
void EditShellMenu(WindowInfo *window)
{
    ...
    Widget nameLabel, cmdLabel, cmdLabel2, dismissBtn, button;
    XmString s1;
    int ac;
    Arg args[20];
    ...
    XtVaSetValues(EditFilterDlog, XmNtitle, "Shell Commands", NULL);
    ...
    XtSetArg(args[ac], XmNbottomPosition, CMD_TOP); ac++;
    ...
    LoadAfterBtn = XtVaCreateManagedWidget("loadAfterBtn",
	    ...
	    XmNbottomPosition, CMD_TOP, NULL);
    XmStringFree(s1);
    SaveFirstBtn = XtVaCreateManagedWidget("saveFirstBtn",
    	    xmToggleButtonWidgetClass, form,
	    ...
	    XmNbottomWidget, LoadAfterBtn, NULL);
    XmStringFree(s1);
    outBox = XtVaCreateManagedWidget("outBox", xmRowColumnWidgetClass, form,
	    ...
	    XmNrightPosition, LIST_LEFT,
	    XmNbottomAttachment, XmATTACH_WIDGET,
	    XmNbottomWidget, SaveFirstBtn,
	    XmNbottomOffset, 4, NULL);
    SameOutBtn = XtVaCreateManagedWidget("sameOutBtn",
    ...
    SelInpBtn = XtVaCreateManagedWidget("selInpBtn", xmToggleButtonWidgetClass,
    	    ...
    	    XmNset, True, NULL);
    XmStringFree(s1);
    WinInpBtn = XtVaCreateManagedWidget("winInpBtn", xmToggleButtonWidgetClass,
    	    inpBox,
    	    ...
    	    XmNset, False, NULL);
    XmStringFree(s1);
    EitherInpBtn = XtVaCreateManagedWidget("eitherInpBtn",
    	    ...
    	    XmNset, False, NULL);
    XmStringFree(s1);
    NoInpBtn = XtVaCreateManagedWidget("noInpBtn", xmToggleButtonWidgetClass,
    	    inpBox,
    ...
    nameLabel = XtVaCreateManagedWidget("nameLabel", xmLabelGadgetClass, form,
    	    ...
    	    XmNrightPosition, LIST_LEFT,
    	    XmNbottomAttachment, XmATTACH_WIDGET,
    	    XmNbottomWidget, NameTextW, NULL);
    XmStringFree(s1);
...
or \"New\" to add a new command to the menu"),
    ...
    cmdLabel = XtVaCreateManagedWidget("cmdLabel", xmLabelGadgetClass, form,
    	    ...
    	    XmNmarginTop, 5,
    	    XmNtopAttachment, XmATTACH_POSITION,
    	    XmNtopPosition, CMD_TOP,
    	    XmNleftAttachment, XmATTACH_POSITION,
    	    XmNleftPosition, 1, NULL);
    XmStringFree(s1);
    cmdLabel2 = XtVaCreateManagedWidget("cmdLabel", xmLabelGadgetClass, form,
    ...
    XtVaSetValues(cmdLabel, XmNuserData, CmdTextW, NULL); /* for mnemonic */
    ...
    XtVaSetValues(form, XmNdefaultButton, AddBtn, NULL);
	...
	XmListCallbackStruct *call_data)
{
    if (call_data->item_position == 1) {
    	XtVaSetValues(XtParent(AddBtn), XmNdefaultButton, AddBtn, NULL);
    	...
    	XtVaSetValues(XtParent(ChangeBtn), XmNdefaultButton, ChangeBtn, NULL);
...
static void filterMenuCB(Widget w, WindowInfo *window, caddr_t call_data) 
{
    int userData, index, success, flags = 0;
    char *text, *filteredText;
    char command[MAX_FILTER_CMD_LEN], fullName[MAXPATHLEN];
    XmTextPosition left, right;
    ...
    if (!substitutePercent(command, FilterList[index]->cmd, fullName,
    	    MAX_FILTER_CMD_LEN)) {
    ...
    if (FilterList[index]->input == FROM_SELECTION) {
	if (!XmTextGetSelectionPosition(window->textArea, &left, &right) ||
		left == right) {
    	    XBell(TheDisplay, 100);
    	    return;
    ...
    } else if (FilterList[index]->input == FROM_WINDOW) {
	text = XmTextGetString(window->textArea);
    	flags |= ACCUMULATE | ERROR_DIALOGS;
    } else if (FilterList[index]->input == FROM_EITHER) {
	if (XmTextGetSelectionPosition(window->textArea, &left, &right)
		&& left != right)
    	    text = GetTextRange(window->textArea, left, right);
    	else
    ...
    if (FilterList[index]->output == TO_DIALOG) {
    	...
    	if (FilterList[index]->input == FROM_WINDOW) {
    	    left = 0;
    	    right = XmTextGetLastPosition(window->lastFocus);
    	} else
	    if (!XmTextGetSelectionPosition(window->textArea, &left, &right))
    ...
    if (FilterList[index]->saveFirst) {
    	if (!SaveWindow(window)) {
    	    if (FilterList[index]->input != FROM_NONE)
    		XtFree(text);
    	    return;
	}
    }
...
static void updateFilterDialog(int selection)
{
    ...
    XtVaSetValues(ListW, XmNitems, stringTable, XmNitemCount, NFilters+1, NULL);
    for (i=0; i<=NFilters; i++)
...
static void updateFilterDlogFields(void)
{
	...
	XmToggleButtonSetState(SelInpBtn, f->input == FROM_SELECTION, False);
	XmToggleButtonSetState(WinInpBtn, f->input == FROM_WINDOW, False);
	XmToggleButtonSetState(EitherInpBtn, f->input == FROM_EITHER, False);
	XmToggleButtonSetState(NoInpBtn, f->input == FROM_NONE, False);
	XmToggleButtonSetState(SameOutBtn, f->output==TO_SAME_WINDOW, False);
	XmToggleButtonSetState(WinOutBtn, f->output==TO_NEW_WINDOW, False);
	XmToggleButtonSetState(DlogOutBtn, f->output==TO_DIALOG, False);
	XmToggleButtonSetState(SaveFirstBtn, f->saveFirst, False);
	XmToggleButtonSetState(LoadAfterBtn, f->loadAfter, False);
    } else {
    	XmTextSetString(NameTextW, "");
	XmTextSetString(CmdTextW, "");
	XmTextSetString(AccTextW, "");
	XmTextSetString(MneTextW, "");
	XmToggleButtonSetState(SelInpBtn, True, True);
	XmToggleButtonSetState(SameOutBtn, True, True);
	XmToggleButtonSetState(SaveFirstBtn, False, False);
	XmToggleButtonSetState(LoadAfterBtn, False, False);
    }
}    
...
static int readFilterDialogFields(filterListRec *f)
{
    ...
    if (nameText == NULL || *nameText == '\0') {
    	DialogF(DF_WARN, EditFilterDlog, 1,
    ...
    if (cmdText == NULL || *cmdText == '\0') {
    	DialogF(DF_WARN, EditFilterDlog, 1, "Please specify a shell command",
    		"OK");
    	XtFree(nameText);
    	if (cmdText!=NULL)
    	    XtFree(cmdText);
    ...
    if (XmToggleButtonGetState(SelInpBtn))
    	f->input = FROM_SELECTION;
    else if (XmToggleButtonGetState(WinInpBtn))
    	f->input = FROM_WINDOW;
    else if (XmToggleButtonGetState(EitherInpBtn))
    	f->input = FROM_EITHER;
    else
    	f->input = FROM_NONE;
    ...
    f->saveFirst = XmToggleButtonGetState(SaveFirstBtn);
    f->loadAfter = XmToggleButtonGetState(LoadAfterBtn);
    return True;
}
	...
	int *stdoutFD, int *stderrFD)
{
    ...
    */
	...
	execl(GetPrefShell(), GetPrefShell(), "-c", command,  NULL);
...
static Widget createWorkingDialog(Widget parent, int *abortFlag)
{
    ...
    XtVaSetValues(XmMessageBoxGetChild(dlg, XmDIALOG_MESSAGE_LABEL),
    	    XmNlabelString,
    	    st=XmStringCreateSimple("Waiting for command to complete"), NULL);
    XtVaSetValues(XtParent(dlg), XmNtitle, " ", NULL);
    XmStringFree(st);
    XtManageChild(dlg);
...
{
    char *outStr, *outPtr, *c, accStr[MAX_FILTER_ACC_LEN];
    filterListRec *f;
    int i, length;
    ...
    for (i=0; i<NFilters; i++) {
    	...
    	*outPtr++ = '\t';
    	for (c=f->cmd; *c!='\0'; c++) { /* Copy the command string, changing */
    	    *outPtr++ = *c;		/* backslashes to double backslashes */
    	    if (*c == '\\')
    	    	*outPtr++ = *c;
    	}
    	*outPtr++ = '\\';
	...
	KeySym *keysym)
{
    ...
    char fields[8][MAX_FILTER_ACC_LEN];
