...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)fontsel.c	1.8	9/26/94";
...
static void	getFontPart(char *font, char *buff1)
{
  ...
  char bigbuff[TEMP_BUF_SIZE*10];

	getStringComponent(font, 2, buff1);
	getStringComponent(font, 1, buff2);

	sprintf(bigbuff, "%s (%s", buff1, buff2);
  memset(buff3, 0, TEMP_BUF_SIZE);
  strncpy(buff3, bigbuff, TEMP_BUF_SIZE-1);
	...
	{
		sprintf(bigbuff, "%s, %s,", buff3, buff1);
    memset(buff2, 0, TEMP_BUF_SIZE);
    strncpy(buff2, bigbuff, TEMP_BUF_SIZE-1);
		sprintf(bigbuff, "%s %s)", buff2, buff4);
    memset(buff1, 0, TEMP_BUF_SIZE);
    strncpy(buff1, bigbuff, TEMP_BUF_SIZE-1);
	}
}
