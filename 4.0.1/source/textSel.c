...
void CopyToClipboard(Widget w, Time time)
{
    ...
    if (XmClipboardCopy(XtDisplay(w), XtWindow(w), itemID, "STRING",
    	    text, strlen(text)+1, 0, NULL) != ClipboardSuccess) {
    	XtFree(text);
    	return;
    }
    XtFree(text);
...
void SendSecondarySelection(Widget w, Time time, int removeAfter)
{
    sendSecondary(w, time, getAtom(XtDisplay(w), A_MOTIF_DESTINATION),
    	    removeAfter ? REMOVE_SECONDARY : UNSELECT_SECONDARY, NULL);
}
...
void InsertClipboard(Widget w, Time time, int isColumnar)
{
    ...
    if (sel->selected && ((TextWidget)w)->text.pendingDelete) {
    	BufReplaceSelected(buf, string);
	...
	Atom *type, XtPointer value, unsigned long *length, int *format)
{
    ...
    string = XtMalloc(*length + 1);
    strncpy(string, (char *)value, *length);
    string[*length] = '\0';
	...
	Boolean *continueDispatch)
{
	...
	BufReplaceSecSelect(buf, cbInfo->actionText);
	if (buf->secondary.rectangular) {
	    ...
	    selEnd = selStart + strlen(cbInfo->actionText);
