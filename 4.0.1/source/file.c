...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)file.c	1.26     10/26/94";
#include <stdio.h>
#include <errno.h>
#include <limits.h>
#include <string.h>
...
void EditNewFile()
{
    ...
    window = CreateWindow(name, GetPrefRows(), GetPrefCols(),
    	    GetPrefAutoIndent(), GetPrefAutoSave(), GetPrefSaveOldVersion(),
    	    GetPrefFontName(), GetPrefFontList(), GetPrefWrap(), False,
    	    GetPrefTabDist(), GetPrefShowMatching());
...
void EditExistingFile(WindowInfo *inWindow, char *name, char *path, int flags)
{
    ...
    /* doOpen does not necessarily set the window title or read-only status */
    if (inWindow != NULL) {
...
static int doOpen(WindowInfo *window, char *name, char *path, int flags)
{
    ...
    /* Check that the length that thebuffer thinks it has is the same
...
int PromptForNewFile(WindowInfo *window, char *prompt, char *fullname)
{
    ...
    if (*window->path != '\0')
    	SetFileDialogDefaultDirectory(window->path);
    retVal = GetNewFilename(window->shell, prompt, fullname);
    if (retVal != GFN_OK)
    	SetFileDialogDefaultDirectory(savedDefaultDir);
    if (savedDefaultDir != NULL)
    	XtFree(savedDefaultDir);
