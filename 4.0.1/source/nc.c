...
int main(int argc, char **argv)
{
    ...
    /* Reconstruct the command line in string commandLine in case we have
       to start a server (nc command line args parallel nedit's).  Special
       characters are protected from the shell by escaping EVERYTHING with \ */
    for (i=1; i<argc; i++)
    	length += 1 + strlen(argv[i])*2;
    commandLine = XtMalloc(length+1);
    outPtr = commandLine;
    for (i=1; i<argc; i++) {
    	for (c=argv[i]; *c!='\0'; c++) {
#ifndef VMS
