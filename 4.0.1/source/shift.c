...
static char *fillParagraph(char *text, int addIndent, int width,
	int startCol, int tabDist, int allowTabs, int *filledLen);
...
void FillSelection(WindowInfo *window)
{
    ...
    int left, right, nCols, leftMargin, len, isRect, rectStart, rectEnd;
    ...
    /* Fill the text */
    filledText = fillParagraph(text, leftMargin, rightMargin-leftMargin,
    	    (hasSelection && isRect) ? rectStart : 0, buf->tabDist, 
    	    buf->useTabs, &len);
    XtFree(text);
    
    /* If this is a rectangular fill, trim the trailing newlines */
    if (hasSelection && isRect) {
	...
	int nChars, int *newLen)
{
    ...
    */
    if (direction == SHIFT_RIGHT)
        ...
        bufLen = strlen(text) + countLines(text) * tabDist + 1;
    ...
    */
	...
	if (*textPtr=='\n' || *textPtr=='\0') {
	    *linePtr = '\0';
	    if (direction == SHIFT_RIGHT)
		shiftLineRight(line, tabsAllowed, tabDist, nChars);
	    else
		shiftLineLeft(line, tabDist, nChars);
...
static void shiftLineRight(char *line, int tabsAllowed, int tabDist, int nChars)
{
...
/*
** Trim leading space, arrange text to fill width "width", and add back
** "addIndent"
*/
static char *fillParagraph(char *text, int addIndent, int width,
	int startCol, int tabDist, int allowTabs, int *filledLen)
    ...
    *outPtr = '\0';
    ...
    for (c=cleanedText; *c!='\0'; c++) {
    	if (*c == '\n')
    	    ...
    	    col += BufCharWidth(*c, col, tabDist);
    	if (col-startCol-1 > width) {
    	    inWhitespace = True;
    ...
    indentString = XtMalloc(sizeof(char) * addIndent + 1);
    outPtr = indentString;
    if (allowTabs) {
	for (i=0; i<addIndent/tabDist; i++)
    	    *outPtr++ = '\t';
	for (i=0; i<addIndent%tabDist; i++)
    	    *outPtr++ = ' ';
    } else {
    	for (i=0; i<addIndent; i++)
    	    *outPtr++ = ' ';
    }
