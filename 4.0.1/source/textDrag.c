...
void BeginBlockDrag(TextWidget tw)
{
    ...
    int nLines, mousePos, mouseLineNum, lineStart;
    int x, y, lineEnd;
    ...
    if (sel->rectangular) {
        ...
        if (!TextDPositionToXY(textD, sel->start, &x, &y))
            x = BufCountDispChars(buf, BufStartOfLine(buf, sel->start),
    ...
    if (!TextDPosToLineNum(textD, mousePos, &mouseLineNum))
    	fprintf(stderr, "NEdit, internal consistency check bbd failed\n");
...
void BlockDragSelection(TextWidget tw, int x, int y, int dragType)
{
    ...
    /* Find the line number of the insert position */
    	...
    	int *rightMargin)
{
    ...
    for (pos=start; pos<end; pos++) {
    	...
    	if (c == '\n') {
    	    ...
    	    width += BufCharWidth(c, width, buf->tabDist);
    }
    *leftMargin = minWhite;
